...
**Important:** the current shared PostgreSQL database name is **`salesbinder`**. One PostgreSQL cache database serves exactly one SalesBinder account. The first writer binds it to the normalized SalesBinder subdomain; later writers for a different account fail before changing cache data. Renaming the local CLI account alias does not change that binding.
...
Cache schema v7 introduced source-API provenance for inventory and category rows. Schema v8 adds PostgreSQL-only inventory change-feed state, baseline staging, and immutable receipt tables while keeping v7 snapshot metadata readable during migration. `cache sync` still requires `v3ApiKey` and never falls back to v2 for inventory or categories. The CLI has two inventory modes: an unbound compatibility snapshot path for SQLite or non-feed caches, and a feed-backed PostgreSQL path for change-feed-bound caches. The compatibility path fetches complete validated v3 snapshots and publishes them atomically; the feed-backed path resumes from a verified baseline, drains change-feed events, and uses exact-ID hydration for touched items. Inventory snapshot metadata format v2 is an internal cache contract for clean and warning outcomes, not SalesBinder API v2. Embedded variation locations retain their names, while a parent item's direct `location_id` has no location name unless the item payload supplies one. CSV-derived stock rows and their numeric values remain unchanged; when a CSV and API item share an ID, the v3 row is the canonical item master. For direct SQL, treat `NULL` as unknown—not as zero—and check the snapshot metadata before treating a cache as authoritative.

Additive inventory columns record `quantity_available_source` on item and stock rows without changing the v8 snapshot or cursor contract. At each exact scope, an observed API `quantity_available` wins; otherwise availability is computed as that row's finite on-hand quantity minus its finite reserved quantity. Missing operands remain `NULL`, signed results are preserved, and parent reservations are never applied to variation rows. CSV values remain unchanged and have `NULL` provenance.
...
# Force a full run (reconciliation for feed-bound PostgreSQL caches; compatibility snapshot for unbound caches)
...
Every cache sync path still requires `v3ApiKey` and there is no v2 fallback for inventory or categories. SQLite and unbound PostgreSQL caches continue to use the compatibility snapshot path: categories and inventory are fetched as complete validated v3 snapshots, pagination metadata must remain coherent across every page, and two consecutive v3 membership reads must agree before publication. Feed-bound PostgreSQL caches use the change-feed path instead: inventory starts from a verified baseline, drains a fixed ledger target with exact-ID hydration, and resumes from durable PostgreSQL receipts and checkpoints. In both modes, the previous category or inventory authority remains unchanged if fetch or validation fails. Item deletion comes from the authoritative source for that mode: compatibility snapshots remove absent API items at atomic publication, while feed mode requires a signed delete event plus an exact v3 `404` before writing a tombstone. V2 deleted-log item tombstones are excluded, so `deleted_records_processed` covers only the remaining deleted-log resources.
...
- Compatibility mode keeps the old snapshot workflow for unbound caches.
