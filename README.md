...
Cache schema v7 records the source API version for inventory and category rows. `cache sync` still requires `v3ApiKey` and never falls back to v2 for inventory or categories. The CLI now has two inventory modes: an unbound compatibility snapshot path for SQLite or non-feed caches, and a feed-backed PostgreSQL path for change-feed-bound caches. The compatibility path fetches complete validated v3 snapshots and publishes them atomically; the feed-backed path resumes from a verified baseline, drains change-feed events, and uses exact-ID hydration for touched items. Inventory snapshot metadata format v2 is an internal cache contract for clean and warning outcomes, not SalesBinder API v2. Embedded variation locations retain their names, while a parent item's direct `location_id` has no location name unless the item payload supplies one. CSV-derived stock rows and their numeric values remain unchanged; when a CSV and API item share an ID, the v3 row is the canonical item master. For direct SQL, treat `NULL` as unknown—not as zero—and check the snapshot metadata before treating a cache as authoritative.
