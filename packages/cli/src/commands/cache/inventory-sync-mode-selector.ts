...
export interface InventorySyncModeSelectorInput {
  ...
  /** Authoritative metadata returned only after cache snapshot verification. */
  baseline: InventoryCacheMeta | null;
...
export function selectInventorySyncMode(
  ...
  return selected('incremental', null, null, input.baseline?.generation ?? null);
}
...
function hasVerifiedBaseline(
  ...
  if (!binding?.baselineGeneration || !baseline) return false;
