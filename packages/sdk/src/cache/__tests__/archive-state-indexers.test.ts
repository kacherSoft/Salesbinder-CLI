...
describe('cache archive-state indexers', () => {
  ...
  it('keeps shipping mappings while removing NUL bytes from cached document text', () => {
    ...
    const document = {
      id: 'doc-shipping', context_id: DocumentContextId.Invoice, document_number: 2,
      customer_id: 'customer-1', user_id: 'user-1', issue_date: '2026-01-01', status_id: 1,
      total_cost: 1, total_tax: 0, total_tax2: 0, total_price: 2, total_transactions: 0,
      created: '2026-01-01', modified: '2026-01-02', archived: false,
      name: `Order${nul}name`, date_sent: '2026-01-03', shipped_percent: 40,
      customer: { name: `Customer${nul}One` },
      document_items: [{
        id: 'line-1', document_id: 'doc-shipping', item_id: 'item-1',
        name: `Item${nul}One`, description: `Line${nul}description`, quantity: 5,
        quantity_partially_received: 1, quantity_partially_shipped: 2,
        tax: 0, tax2: 0, discount_percent: 0, cost: 1, price: 2,
        discounted_price: 2, weight: 0, created: '2026-01-01', modified: '2026-01-02',
      }],
    } as Document;

    const { docRow, itemRows } = (indexer as any).processDocument(document);

    expect(docRow).toMatchObject({
      document_name: 'Ordername', account_name: 'CustomerOne', date_sent: '2026-01-03',
      shipped_percent: 40, archived: 0,
    });
    expect(itemRows[0]).toMatchObject({
      item_name: 'ItemOne', line_description: 'Linedescription', quantity_shipped: 2,
    });
  });

  it('refreshes invoice payments without dropping shipping or archive fields', async () => {
    const invoice = {
      id: 'invoice-1', context_id: DocumentContextId.Invoice, document_number: 3,
      customer_id: 'customer-1', user_id: 'user-1', issue_date: '2026-01-01', status_id: 1,
      total_cost: 1, total_tax: 0, total_tax2: 0, total_price: 2, total_transactions: 0,
      created: '2026-01-01', modified: '2026-01-02', archived: true,
      date_sent: '2026-01-03', shipped_percent: 50, transactions: [],
      document_items: [{
        id: 'line-1', document_id: 'invoice-1', item_id: 'item-1', quantity: 5,
        quantity_partially_received: 1, quantity_partially_shipped: 3,
        tax: 0, tax2: 0, discount_percent: 0, cost: 1, price: 2,
        discounted_price: 2, weight: 0, created: '2026-01-01', modified: '2026-01-02',
      }],
    } as Document;
    ...
    const cacheWithPayments = {
      ...
      getPaymentSyncStatus: jest.fn(async () => ({
        status: 'complete', startedAt: 1, updatedAt: 1, finishedAt: 1,
        lastSuccessfulSync: 1, cursor: 'old', processedDocuments: 1, totalDocuments: 1,
      })),
      ...
      insertDocument: jest.fn(async (row) => { insertedDocuments.push(row); }),
      batchInsertItemDocuments: jest.fn(async (rows) => { insertedItems.push(...rows); }),
      replacePaymentTransactions: jest.fn(async () => undefined),
    ...
    expect(insertedDocuments[0]).toMatchObject({
      date_sent: '2026-01-03', shipped_percent: 50, archived: 1,
    });
    expect(insertedItems[0]).toMatchObject({ quantity_shipped: 3 });
    expect(cacheWithPayments.replacePaymentTransactions).toHaveBeenCalledWith('invoice-1', []);
  ...
  it('leaves global watermarks for the CLI while using the shared schema version', async () => {
    ...
    for (const state of states) {
      expect(state).toMatchObject({
        lastSync: 0, lastFullSync: 0, schemaVersion: CACHE_SCHEMA_VERSION,
      });
    }
  });
  ...
  it('advances direct document watermarks but lets the cache pipeline defer them', async () => {
    ...
    await new DocumentIndexerService(
      emptyClient, stateCache, 'test', undefined, undefined, { deferGlobalWatermark: true },
    ).sync({ full: true });

    expect(states[0]).toMatchObject({
      lastSync: expect.any(Number), lastFullSync: expect.any(Number),
      documentCount: 7, itemDocumentCount: 8,
    });
    expect(states[0].lastSync).toBeGreaterThan(0);
    expect(states[0].lastFullSync).toBeGreaterThan(0);
    expect(states[1]).toMatchObject({
      lastSync: 0, lastFullSync: 0, documentCount: 7, itemDocumentCount: 8,
    });
  });
});
